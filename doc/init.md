...
All three startup configurations assume the existence of a "oneworld" user
...
If oneworldd is run with "-daemon" flag, and no rpcpassword is set, it will
print a randomly generated suitable password to stderr.  You can also
generate one from the shell yourself like this:

bash -c 'tr -dc a-zA-Z0-9 < /dev/urandom | head -c32 && echo'

Once you have a password in hand, set rpcpassword= in /etc/oneworld/oneworld.conf
...
PID file:            /var/run/oneworldd/oneworldd.pid (OpenRC and Upstart)
                     /var/lib/oneworldd/oneworldd.pid (systemd)
